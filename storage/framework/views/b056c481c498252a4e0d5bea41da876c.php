      ...
      <form method="POST" action="<?php echo e(route('products.update', $product->id)); ?>" enctype="multipart/form-data">
        ...
        <div style="margin-bottom:1rem">
          ...
          <select name="category"
                  style="width:100%;padding:.8rem;border:1px solid #ddd;border-radius:8px;font-family:inherit">
            <option value="coffee"    <?php echo e(old('category', $product->category)=='coffee'    ? 'selected' : ''); ?>>☕ Coffee</option>
            <option value="noncoffee" <?php echo e(old('category', $product->category)=='noncoffee' ? 'selected' : ''); ?>>🧋 Non-Coffee</option>
            <option value="bakery"    <?php echo e(old('category', $product->category)=='bakery'    ? 'selected' : ''); ?>>🥐 Bakery</option>
            <option value="food"      <?php echo e(old('category', $product->category)=='food'      ? 'selected' : ''); ?>>🍽️ Food</option>
          </select>
        </div>
        ...
        <div style="margin-bottom:1rem">
          ...
          <textarea name="description" rows="3"
                    style="width:100%;padding:.8rem;border:1px solid #ddd;border-radius:8px;font-family:inherit"><?php echo e(old('description', $product->description)); ?></textarea>
        </div>

        <div style="margin-bottom:1rem">
          <label style="display:block;margin-bottom:.5rem;font-weight:500">ราคา (บาท) *</label>
          <input type="number" name="price" value="<?php echo e(old('price', $product->price)); ?>" min="0" step="0.01"
                 style="width:100%;padding:.8rem;border:1px solid #ddd;border-radius:8px;font-family:inherit"
                 required />
        </div>
        ...
        <div style="margin-bottom:1rem">
          ...
          <?php if($product->image): ?>
            <div style="margin-bottom:.5rem;">
              <img src="<?php echo e(asset('storage/' . $product->image)); ?>" alt="Current Image" style="height:100px; border-radius:8px; object-fit:contain; background:var(--cream); padding:5px;">
            </div>
          <?php endif; ?>
...
</html><?php /**PATH /var/www/html/resources/views/dashboard/products/edit.blade.php ENDPATH**/ ?>